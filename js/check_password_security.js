...
passwordValidation: function (_options, _callback, _confirmcallback) {
    ...
    maxRepeats: 4, //Maximum number of repeated alphanumeric characters in password dhgurAAAfjewd <- 3 A's
    ...
    return (
        CHARSETS.upperCaseSet +
        CHARSETS.lowerCaseSet +
        CHARSETS.digitSet +
        CHARSETS.specialSet
    );
    }
    ...
    var _element = this;
    var $confirmField =
    _options.confirmField != undefined
        ? $(_options.confirmField)
        : undefined;
    ...
    if (
        _options.failRepeats &&
        $(_element)
        .val()
        .search(
            new RegExp("(.)" + ".*\\1".repeat(_options.maxRepeats - 1), "gi")
        ) != -1
    ) {
        failedCases.push(
        "Password can not contain " +
            _options.maxRepeats +
            " of the same character case insensitive."
        );
    }
    if (
        _options.failConsecutive &&
        $(_element)
        .val()
        .search(
            new RegExp(
            "(?=(.)" + "\\1".repeat(_options.maxConsecutive) + ")",
            "g"
            )
        ) != -1
    ) {
        failedCases.push(
        "Password can't contain the same character more than " +
    ...
    var validPassword =
        ...
        $(_element).val().length >= _options.minLength;
    ...
    if ($confirmField != undefined) {
        $confirmField.bind(
        "keyup focus input proprtychange mouseup",
        validateField
        );
    }
    });
...
(function ($) {
$.fn.extend({
usernameValidation: function (_options, _callback, _confirmcallback) {
    //var _unicodeSpecialSet = "^\\x00-\\x1F\\x7F\\x80-\\x9F0-9A-Za-z"; //All chars other than above (and C0/C1)
    var CHARSETS = {
    upperCaseSet: "A-Z", //All UpperCase (Acii/Unicode)
    lowerCaseSet: "a-z", //All LowerCase (Acii/Unicode)
    digitSet: "0-9", //All digits (Acii/Unicode)
    specialSet: "\\x2F\\x3A-\\x40\\x5B-\\x60\\x7B-\\x7E\\x80-\\xFF", //All Other printable Ascii
    };
    var _defaults = {
    minLength: 8, //Minimum Length of password
    maxRepeats: 4, //Maximum number of repeated alphanumeric characters in password dhgurAAAfjewd <- 3 A's
    ...
    if (_options.noLower)
        cases.push({
          regex: "(?=" + CHARSETS.lowerCaseSet + ")",
          message: "Username can't contain a Lower Case Letter",
        });
    else
        cases.push({
          regex:
    ...
    if (_options.noDigit)
        cases.push({
          regex: "(?=" + CHARSETS.digitSet + ")",
          message: "Username can't contain a Number",
        });
    else
        cases.push({
          regex:
            "(?=" +
            ("[" + CHARSETS.digitSet + "][^" + CHARSETS.digitSet + "]*").repeat(
              _options.minDigits
            ) +
            ")",
          message:
            "Username must contain at least " + _options.minDigits + " Digits.",
        });
    if (_options.noSpecial)
        cases.push({
          regex: "(?=" + CHARSETS.specialSet + ")",
          message: "Username can't contain a Special Character",
        });
    else
        cases.push({
          regex:
    ...
    cases.push({
      regex: "[" + charsetToString() + "]{" + _options.minLength + ",}",
      message:
        "Username must contain at least " + _options.minLength + " characters.",
    });
    ...
    if (
        ...
        failedCases.push(
          "Username can't contain the same character more than " +
            _options.maxConsecutive +
            " times in a row."
        );
    }
